...
def script_paths_to_dict(paths):
    ...
    for path in paths:
        command_name = \
            os.path.basename(path).replace('panplot-', '').replace('.py', '')
...
class MultiCommand(click.MultiCommand):
    ...
    def get_command(self, ctx, name):
        ...
        from panplot.commands.external import external_cli as cli
        from panplot.commands.external import get_command_help
...
def query_option(**attrs):
    ...
    def decorator(f):
        ...
        attrs.setdefault(
            'default',
            lambda: panplot.config.get('default-query-string')
        )
...
def git_option(**attrs):
    ...
    def decorator(f):
        ...
        attrs.setdefault(
            'default',
            lambda: True if panplot.config.get('use-git') else False
        )
